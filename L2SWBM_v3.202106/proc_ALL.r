...
superiorOutflow_Prior = getSubset(superiorOutflow_Prior, priorStartYear, priorStartMonth, priorEndYear, priorEndMonth)
...
miHuronOutflow_Prior = getSubset(miHuronOutflow_Prior, priorStartYear, priorStartMonth, priorEndYear, priorEndMonth)
...
clairOutflow_Prior = getSubset(clairOutflow_Prior, priorStartYear, priorStartMonth, priorEndYear, priorEndMonth)

erieOutflow_Prior = getSubset(erieOutflow_Prior, priorStartYear, priorStartMonth, priorEndYear, priorEndMonth)
...
ontarioOutflow_Prior = getSubset(ontarioOutflow_Prior, priorStartYear, priorStartMonth, priorEndYear, priorEndMonth)
...
clairNBS_Prior[is.na(clairNBS_Prior[,3]),3] = clairNBS_Prior[is.na(clairNBS_Prior[,3]),ncol(clairNBS)]
...
for(i in 1:12){
	...
	## Outflow
	superiorOutflowPriorMean[i] =  mean(superiorOutflow_Prior[superiorOutflow_Prior[,2]==i,4], na.rm=TRUE)
	superiorOutflowPriorPrecision[i] = (1/var(superiorOutflow_Prior[superiorOutflow_Prior[,2]==i,4], na.rm=TRUE))					
	miHuronOutflowPriorMean[i] =  mean(miHuronOutflow_Prior[miHuronOutflow_Prior[,2]==i,4], na.rm=TRUE)
	miHuronOutflowPriorPrecision[i] = (1/var(miHuronOutflow_Prior[miHuronOutflow_Prior[,2]==i,4], na.rm=TRUE))
	...
	clairOutflowPriorMean[i] =  mean(clairOutflow_Prior[clairOutflow_Prior[,2]==i,4], na.rm=TRUE)
	clairOutflowPriorPrecision[i] = (1/var(clairOutflow_Prior[clairOutflow_Prior[,2]==i,4], na.rm=TRUE))	
...
if(dHPrecDefined){
	inputDataCoreJAGS[['dHPrec']] = dHPrec
}
...
if(length(names(yPrecip)) > 0){
	for(n in 1:length(names(yPrecip))){
		...
		modelCode = paste(
			modelCode,
			'\t\t',names(yPrecip)[n],'Bias[i] ~ dnorm(0,0.001)\n',
			sep=''
		);
	}
}
...
for(n in 1:length(names(yNBS))){	
	modelCode = paste(
		modelCode,
		'\t\t',names(yNBS)[n],'Bias[i] ~ dnorm(0,0.053) # For CMS equivalent\n',
		sep=''
	);
}
...
modelCode = paste(modelCode,
'
		ySuperiorOutflow1Prec[h] <- ySuperiorOutflow1PrecMain      
		ySuperiorOutflow2Prec[h] <- ySuperiorOutflow2PrecMain
		ySuperiorDiversion1Prec[h] <- ySuperiorDiversion1PrecMain
		yMiHuronOutflow1Prec[h] <- yMiHuronOutflow1PrecMain      
		yMiHuronOutflow2Prec[h] <- yMiHuronOutflow2PrecMain
		yMiHuronDiversion1Prec[h] <- yMiHuronDiversion1PrecMain
		yClairOutflow1Prec[h] <- yClairOutflow1PrecMain
		yClairOutflow2Prec[h] <- yClairOutflow2PrecMain
		yErieOutflow1Prec[h] <- yErieOutflow1PrecMain  
		yErieDiversion1Prec[h] <- yErieDiversion1PrecMain
		yOntarioOutflow1Prec[h] <-	yOntarioOutflow1PrecMain

	} # END FLOW AND DIVERSION SPECIFICATION
		
	ySuperiorOutflow1PrecMain ~ dgamma(0.1,0.1)      
	ySuperiorOutflow2PrecMain ~ dgamma(0.1,0.1)
	ySuperiorDiversion1PrecMain ~ dgamma(0.1,0.1)
	yMiHuronOutflow1PrecMain ~ dgamma(0.1,0.1)      
	yMiHuronOutflow2PrecMain ~ dgamma(0.1,0.1)
	yMiHuronDiversion1PrecMain ~ dgamma(0.1,0.1)
	yClairOutflow1PrecMain ~ dgamma(0.1,0.1)
	yClairOutflow2PrecMain ~ dgamma(0.1,0.1)
	yErieOutflow1PrecMain ~ dgamma(0.1,0.1)  
	yErieDiversion1PrecMain ~ dgamma(0.1,0.1)
	yOntarioOutflow1PrecMain ~ dgamma(0.1,0.1)	
',
sep=''
)
}else{
...
if(biasOutflows){
	paramsToMonitor = c(paramsToMonitor,
		"ySuperiorOutflow1PrecMain",      
		"ySuperiorOutflow2PrecMain",
		"ySuperiorDiversion1PrecMain",
		"yMiHuronOutflow1PrecMain",      
		"yMiHuronOutflow2PrecMain",
		"yMiHuronDiversion1PrecMain",
		"yClairOutflow1PrecMain",
		"yClairOutflow2PrecMain",
		"yErieOutflow1PrecMain",  
		"yErieDiversion1PrecMain",
		"yOntarioOutflow1PrecMain"	
	)
}
...
jMod = jags.model(
	file = paste('L2SWBM_ALL_',rollPeriod,'_',
		as.numeric(biasOutflows),
		as.numeric(incProcError),
		as.numeric(checkModel),		
		as.numeric(dHPrecDefined),
	'_',modelSuffix,'.bug.r', sep=''),
